...
export default function FriendsScreen() {
  ...
  const setFriendRequestCount = useUserStore(
  ...
  const [suppressIncomingAnim, setSuppressIncomingAnim] =
    useState(false);
  ...
  const loadRequests = useCallback(async () => {
    ...
    const [incomingData, sentData, suggestionsData] =
      await Promise.all([
  ...
  function handleViewProfile(targetId: string) {
    if (!targetId) return;

    router.push(`/friend/${targetId}`);
  }
      ...
      {showingSearch ? (
            ...
            <View style={styles.row}>
              ...
              {item.relation === 'none' && (
                ...
                >
                  ...
                  <Text style={styles.quickAddPillText}>
                    {pendingIds.has(item.id)
                      ? 'Added'
                      : 'Add'}
                  </Text>
        ...
        <ScrollView contentContainerStyle={styles.list}>
          ...
          {incoming.length > 0 && (
            <>
              ...
              {(
                ...
                const extraIndex =
                  i - INCOMING_COLLAPSED_COUNT;

                return (
                  <Animated.View
                    key={req.id}
                    layout={
                      suppressIncomingAnim
                        ? undefined
          ...
          {incoming.length === 0 &&
            ...
            suggestions.length === 0 && (
              <Text style={styles.emptyText}>
                No pending requests. Search above to add
                a friend.
              </Text>
            )}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // NEW:
  // Makes the avatar + name + level tappable
  // without interfering with Add/Accept/Cancel.
  profileArea: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
